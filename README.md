...
```bash
roslaunch adi_3dtof_floor_detector adi_3dtof_floor_detector.launch arg_input_mode:=0
```
...
```bash
cd ~/catkin_ws/src/adi_3dtof_floor_detector
git pull
cd ~/catkin_ws
...
1. Go to the installation directory of the ADI 3DToF ADTF31xx application `~/Analog Devices/ADI3DToFFloorDetector-Rel1.1.0`
...
4. Update the input file argument `arg_input_file_name` in the launch file `adi_3dtof_adtf31xx.launch` as per the above file path.
...
# Launch the package
$ roslaunch adi_3dtof_floor_detector adi_3dtof_floor_detector.launch arg_input_mode:=2
```
...
```bash
roslaunch adi_3dtof_floor_detector adi_3dtof_floor_detector.launch arg_input_mode:=3 arg_input_sensor_ip:=10.43.0.1
```
> [!note]
...
| **arg_config_file_name_of_tof_sdk**           | String   | "config/config_adsd3500_adsd3100.json" | Configuration file name of ToF SDK: _"config_adsd3500_adsd3100.json" - For ADSD3100 Devices, "config_adsd3500_adsd3030.json" - For ADSD3030 Devices_    |
...
$ git clone https://github.com/analogdevicesinc/adi_3dtof_floor_detector.git -b v1.1.0
```
2. Update submodules in aditof SDK
```bash
...
5. This node can be run in 2 ways using the following command in a new terminal. 
...
| Topic                         | Description                                    |
|-------------------------------|------------------------------------------------|
| **/floor_marked_depth_image** | 8-bit RGB image                             |
| **/floor_removed_depth_image**| 16-bit Depth image                                |
